...
import { Plus, Search, Edit2, Trash2, Download, Eye, RotateCcw } from 'lucide-react';
import { useNavigate } from 'react-router-dom';

const returns = [
  { id: 'SR-2024-001', date: '08/03/2024', invoice: 'INV-2024-001', customer: 'Ramesh Patel', amount: 5000, status: 'Processed' },
  { id: 'SR-2024-002', date: '15/03/2024', invoice: 'INV-2024-002', customer: 'Kisan Traders', amount: 12000, status: 'Pending' },
];
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm flex-1 flex flex-col overflow-hidden">
        <div className="p-4 border-b border-gray-200 flex items-center justify-between bg-gray-50/50">
          ...
          <div className="flex gap-2">
            ...
            <button className="px-3 py-2 border border-gray-300 rounded-md text-sm font-medium hover:bg-gray-50 text-gray-700 transition-colors bg-white flex items-center gap-2" onClick={() => exportToCSV(returns, 'SalesReturns')}>
            ...
            <tbody className="divide-y divide-gray-200">
              {returns.map((ret) => (
                <tr key={ret.id} className="hover:bg-gray-50 transition-colors group">
                  <td className="p-4 text-sm font-medium text-blue-600">{ret.id}</td>
                  <td className="p-4 text-sm text-gray-600">{ret.date}</td>
                  <td className="p-4 text-sm text-gray-600">{ret.invoice}</td>
                  <td className="p-4 text-sm text-gray-900 font-medium">{ret.customer}</td>
                  <td className="p-4 text-sm text-gray-900 font-mono text-right">{ret.amount.toLocaleString('en-IN')}</td>
                  <td className="p-4 text-sm">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                      ret.status === 'Processed' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'
                    }`}>
                      {ret.status}
                    </span>
                  </td>
                  <td className="p-4 flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button className="text-gray-400 hover:text-green-600 transition-colors" title="Process Credit Note">
