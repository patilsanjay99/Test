...
export function QuotationPrint() {
  ...
  useEffect(() => {
    const fetchQuotation = async () => {
      try {
        const res = await fetch(`/api/data/SalesQuotations`);
        if (!res.ok) throw new Error('Not found');
        const data = await res.json();
        const qt = Array.isArray(data) ? data.find((q: any) => q.Id == id) : null;
        ...
        if (qt.CustomerId) {
          ...
          if (custRes.ok) {
            ...
            const cust = Array.isArray(customers) ? customers.find((c: any) => c.Id == qt.CustomerId) : null;
  ...
  let lines = [];
  try {
    lines = quotation.ItemsData ? JSON.parse(quotation.ItemsData) : [];
  } catch (e) {}
    ...
    <div className="max-w-full mx-auto px-4 lg:px-8 w-full pb-12 print:pb-0 print:m-0 print:max-w-none print:w-full">
      ...
      <div className="bg-white border border-gray-300 print:border-none p-8 sm:p-12 print:p-10 min-h-[1056px] print:min-h-0 print:h-[100vh] print:max-h-[100vh] print:overflow-hidden w-full mx-auto relative text-black text-sm box-border flex flex-col">
        ...
        <div className="text-center border-b border-gray-300 pb-4 mb-6 relative min-h-[60px] flex items-center justify-center">
          ...
          <h1 className="text-2xl font-bold tracking-tight uppercase">SALES QUOTATION</h1>
        </div>
        ...
        <div className="flex justify-between items-start mb-8 gap-8">
          ...
          <div className="flex-1 text-right">
            <div className="grid grid-cols-[110px_1fr] gap-x-4 gap-y-1 text-left inline-grid w-full max-w-[320px] ml-auto">
              <span className="font-semibold text-gray-600 border-r border-gray-300 mr-4">Quote No:</span>
              <span className="font-bold whitespace-nowrap">{quotation.QuotationNumber || 'N/A'}</span>
              
              <span className="font-semibold text-gray-600 border-r border-gray-300 mr-4 mt-2">Date:</span>
              <span className="mt-2">{formatDate(quotation.QuotationDate)}</span>
            </div>
          </div>
        </div>
        ...
        {quotation.TermsAndConditions && (
          <div className="mb-8">
            ...
            <div className="text-gray-600 space-y-1 text-xs whitespace-pre-wrap">{quotation.TermsAndConditions}</div>
          </div>
        )}
