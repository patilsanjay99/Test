...
export function SalesQuotationForm() {
    ...
    if (activeCompany?.id) {
        ...
        if (id) {
          fetch(`/api/data/SalesQuotations?CompanyId=${activeCompany.id}`)
            .then(res => res.json())
            .then(data => {
                const quotation = Array.isArray(data) ? data.find(q => q.Id == id) : null;
                if (quotation) {
                   setQuotationNumber(quotation.QuotationNumber || '');
                   setSelectedCustomer(quotation.CustomerId);
                   setQuoteDate(quotation.QuotationDate.split('/').reverse().join('-'));
                   setValidUntil(quotation.ValidUntil.split('/').reverse().join('-'));
                   if (quotation.ItemsData) setLines(JSON.parse(quotation.ItemsData));
            ...
            .catch(console.error);
             ...
             fetch(`/api/data/Companies?Id=${activeCompany.id}`)
              ...
              .then(data => {
                  ...
                  if (company && company.DefaultQuotationTerms) {
                      setTermsAndConditions(company.DefaultQuotationTerms);
                  }
              })
              .catch(console.error);
        }
  ...
  const saveQuotation = async () => {
    ...
    const data = {
        ...
        QuotationDate: formatDate(quoteDate),
    ...
    try {
        const url = id ? `/api/data/SalesQuotations/${id}` : '/api/data/SalesQuotations';
    ...
    <div className="max-w-full mx-auto px-4 lg:px-8 w-full flex flex-col space-y-6 pb-12 select-none font-sans">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900 tracking-tight">{id ? `Edit Quotation - ${quotationNumber}` : 'Create Quotation'}</h1>
            <p className="text-sm text-gray-500 mt-1">Prepare a{id ? 'n' : ' new'} estimate for a prospective sale.</p>
          </div>
        </div>
      </div>

      <form onSubmit={(e) => { e.preventDefault(); saveQuotation(); }} className="bg-[#f1f5f9] border border-[#8faad8] rounded-lg shadow-md overflow-hidden block">
        {/* Green Title Header */}
        <div className="bg-[#0b8a1c] text-white py-3 px-4 border-b border-blue-900 text-center font-bold text-xl tracking-wide uppercase flex items-center justify-center gap-2">
          <FileText className="w-5 h-5" /> SALES QUOTATION MASTER
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 md:divide-x-2 md:divide-[#8faad8]">
            ...
            <div className="flex flex-col">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 md:border-b-0 border-b border-blue-900 min-h-[48px] items-stretch">
                <div className="bg-[#f1f5f9] px-4 py-3 flex items-center font-bold text-[#1e293b] text-sm sm:col-span-1 border-r border-[#8faad8]">
                  Quotation Date <span className="text-red-500 ml-1">*</span>
                </div>
                <div className="bg-[#f1f5f9] p-1.5 sm:col-span-2 flex items-center">
                  <div className="w-full max-w-[180px]">
                    <CustomDatePicker required value={quoteDate} onChange={setQuoteDate} className="w-full font-mono !py-1" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col">
              <div className="grid grid-cols-1 sm:grid-cols-3 border-b border-blue-900 min-h-[48px] items-stretch">
                ...
                <div className="bg-[#f1f5f9] p-1.5 sm:col-span-2 flex items-center">
                  <div className="w-full max-w-[180px]">
                    <CustomDatePicker required value={validUntil} onChange={setValidUntil} className="w-full font-mono !py-1" />
                  </div>
                </div>
              </div>
        ...
        <div className="bg-[#f1f5f9] border-t-2 border-blue-900 p-4 flex justify-end gap-3">
          ...
          >
            ...
            {saving ? 'SAVING...' : (id ? 'UPDATE QUOTATION' : 'SEND TO CUSTOMER')}
          </button>
        </div>
      </form>
    </div>
