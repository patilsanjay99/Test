...
import { Plus, Search, Edit2, Trash2, Printer, X, Wallet, ArrowRightLeft, Calendar } from 'lucide-react';
...
export function CashPayments() {
  ...
  // Printing state
  const [printPayment, setPrintPayment] = useState<CashPayment | null>(null);
  const printRef = useRef<HTMLDivElement>(null);

  // Load lookup accounts & cash vouchers
  useEffect(() => {
  ...
  const fetchLookupAndPayments = async () => {
    ...
    try {
      ...
      const paymentsRes = await fetch(`/api/v1/data/CashPayments?CompanyId=${companyId}`);
      if (paymentsRes.ok) {
      ...
      console.error('Error fetching cash payments data:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const canAdd = hasPermission('Accounting: Cash Payments', 'add');
  const canEdit = hasPermission('Accounting: Cash Payments', 'edit');
  const canDelete = hasPermission('Accounting: Cash Payments', 'delete');

  const handleDelete = async (id: number) => {
    if (!window.confirm('Are you sure you want to delete this payment voucher? This will also remove its corresponding general ledger journal postings.')) {
      return;
    }
    try {
      const res = await fetch(`/api/v1/data/CashPayments/${id}`, {
        method: 'DELETE',
      });
      if (res.ok) {
  ...
  const filteredPayments = useMemo(() => {
    return payments.filter(p => {
      // Date range filter
      if (startDate) {
        if (p.PaymentDate < startDate) return false;
      }
      if (endDate) {
        if (p.PaymentDate > endDate) return false;
      }
      ...
      if (searchTerm) {
        ...
        const creditAccName = (accountMap.get(p.CashAccountId) || '').toLowerCase();
  ...
  const handlePrint = (p: CashPayment) => {
    setPrintPayment(p);
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Record, configure prefixes, and post full-fledged cash settlements to Ledger accounts.
          </p>
        </div>
        {canAdd && (
          <button
            id="btn-add-cashpayment"
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        {loading ? (
          <div className="p-12 text-center text-gray-500">
            ...
            <p>Loading cash payment records...</p>
          </div>
        ) : filteredPayments.length === 0 ? (
          <div className="p-12 text-center text-gray-400">
            <Wallet className="h-12 w-12 mx-auto text-gray-300 mb-3" />
            <p className="text-base font-medium">No Cash Payments Recorded</p>
            <p className="text-sm mt-1">Try expanding your search parameters or log a new Cash Payment.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200 text-xs font-semibold text-gray-700 uppercase tracking-wider">
                  ...
                  <th className="py-4 px-6">Debit Account</th>
                  ...
                  return (
                    <tr key={paymentId} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="py-4 px-6 text-gray-900 font-medium">
                        {accountMap.get(p.AccountId) || `Debited Acc (${p.AccountId})`}
                      </td>
                      ...
                      <td className="py-3 px-6 text-center">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          {canEdit && (
                            <button
                              id={`btn-edit-${paymentId}`}
                              onClick={() => navigate(`/accounting/payments/${paymentId}`)}
                              title="Edit Voucher"
      ...
      {printPayment && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full flex flex-col max-h-[90vh]">
            ...
            {/* Print Content Container (Matches A4/Letter block structure for pristine vouchers) */}
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4 py-6 border-b border-gray-200 text-sm text-gray-800">
                  <div>
                    ...
                    <strong className="font-mono text-emerald-700 text-base">{printPayment.VoucherNo}</strong>
                  </div>
                ...
                <div className="py-6 border-b border-gray-200 space-y-4">
                  <div className="flex justify-between text-sm">
                    <div className="space-y-1">
                      <span className="text-gray-500 text-xs block font-semibold uppercase">Debited Account (Paid to)</span>
                  ...
                  <div className="flex justify-between text-sm">
                    <div className="space-y-1">
                      <span className="text-gray-500 text-xs block font-semibold uppercase">Credited Account (Paid from)</span>
                      <strong className="text-gray-900 text-base">
                        {accountMap.get(printPayment.CashAccountId) || `Cash/Bank Account (${printPayment.CashAccountId})`}
                      </strong>
                    </div>
                  </div>
                ...
                <div className="py-4 flex justify-between items-center border-b-2 border-gray-900">
                  <span className="text-gray-900 font-bold uppercase text-xs tracking-wider">Total Handover Amount</span>
                  <div className="text-right">
