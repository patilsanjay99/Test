...
import { Plus, Search, Edit2, Trash2, Printer, X, Wallet, Calendar } from 'lucide-react';
...
export function CashReceipts() {
  ...
  // Printing state
  const [printReceipt, setPrintReceipt] = useState<CashReceipt | null>(null);
  const printRef = useRef<HTMLDivElement>(null);

  // Load lookup accounts & cash receipt vouchers
  useEffect(() => {
  ...
  const fetchLookupAndReceipts = async () => {
    ...
    try {
      ...
      // 2. Fetch Cash Receipts
      const receiptsRes = await fetch(`/api/v1/data/CashReceipts?CompanyId=${companyId}`);
      if (receiptsRes.ok) {
        const payData = await receiptsRes.json();
        // Filter by active company
        const filtered = payData.filter((p: any) => Number(p.CompanyId ?? p.companyid ?? 0) === companyId);
        setReceipts(filtered);
      }
    } catch (err) {
      console.error('Error fetching cash receipts data:', err);
    } finally {
      setLoading(false);
    }
  };

  // Maps for account name lookup
  ...
  // Permissions corresponding to Accounting: Cash Receipts (fallback to general check)
  const canAdd = hasPermission('Accounting: Cash Receipts', 'add') || hasPermission('Accounting: Cash Payments', 'add');
  const canEdit = hasPermission('Accounting: Cash Receipts', 'edit') || hasPermission('Accounting: Cash Payments', 'edit');
  const canDelete = hasPermission('Accounting: Cash Receipts', 'delete') || hasPermission('Accounting: Cash Payments', 'delete');

  const handleDelete = async (id: number) => {
    if (!window.confirm('Are you sure you want to delete this receipt voucher? This will also remove its corresponding general ledger journal postings.')) {
      return;
    }
    try {
      const res = await fetch(`/api/v1/data/CashReceipts/${id}`, {
        method: 'DELETE',
      });
      if (res.ok) {
  ...
  const filteredReceipts = useMemo(() => {
    return receipts.filter(p => {
      // Date range filter
      if (startDate) {
        if (p.ReceiptDate < startDate) return false;
      }
      if (endDate) {
        if (p.ReceiptDate > endDate) return false;
      }

      // Search term filter (Voucher No, Narration, account names)
      if (searchTerm) {
        ...
        const debitAccName = (accountMap.get(p.CashAccountId) || '').toLowerCase();
  ...
  const handlePrint = (p: CashReceipt) => {
    setPrintReceipt(p);
  };
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Record, configure prefixes, and post full-fledged cash collections to Ledger accounts.
          </p>
        </div>
        {canAdd && (
          <button
            id="btn-add-cashreceipt"
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        {loading ? (
          <div className="p-12 text-center text-gray-500">
            ...
            <p>Loading cash receipt records...</p>
          </div>
        ) : filteredReceipts.length === 0 ? (
          <div className="p-12 text-center text-gray-400">
            <Wallet className="h-12 w-12 mx-auto text-gray-300 mb-3" />
            <p className="text-base font-medium">No Cash Receipts Recorded</p>
            <p className="text-sm mt-1">Try expanding your search parameters or log a new Cash Receipt.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200 text-xs font-semibold text-gray-700 uppercase tracking-wider">
                  ...
                  <th className="py-4 px-6">Credit Account</th>
                  <th className="py-4 px-6">Narration / Description</th>
                  ...
                  return (
                    <tr key={receiptId} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="py-4 px-6 text-gray-900 font-medium">
                        {accountMap.get(p.AccountId) || `Credited Acc (${p.AccountId})`}
                      </td>
                      <td className="py-4 px-6 text-gray-500 max-w-xs truncate" title={p.Narration}>{p.Narration || '-'}</td>
                      <td className="py-4 px-6 text-right font-bold text-gray-900">₹{p.Amount.toFixed(2)}</td>
                      <td className="py-3 px-6 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <button
                            id={`btn-print-${receiptId}`}
                          ...
                          {canEdit && (
                            <button
                              id={`btn-edit-${receiptId}`}
                              onClick={() => navigate(`/accounting/receipts/${receiptId}`)}
                              title="Edit Voucher"
      ...
      {/* Printable Preview Overlay Frame */}
      {printReceipt && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full flex flex-col max-h-[90vh]">
            ...
            <div className="p-8 overflow-y-auto flex-1 bg-gray-50">
              <div 
                ref={printRef}
                className="bg-white border-2 border-dashed border-gray-300 p-8 rounded-lg shadow-sm"
              >
                {/* Header organization name stamp */}
                ...
                {/* Meta details segment (Voucher Number, Dates, FYs) */}
                ...
                {/* Ledger posting breakdown columns */}
                <div className="py-6 border-b border-gray-200 space-y-4">
                  <div className="flex justify-between text-sm">
                    <div className="space-y-1">
                      <span className="text-gray-500 text-xs block font-semibold uppercase">Credited Account (Received from)</span>
                  ...
                  <div className="flex justify-between text-sm">
                    <div className="space-y-1">
                      <span className="text-gray-500 text-xs block font-semibold uppercase">Debited Account (Received in)</span>
                      <strong className="text-gray-900 text-base">
                        {accountMap.get(printReceipt.CashAccountId) || `Cash/Bank Account (${printReceipt.CashAccountId})`}
                      </strong>
                    </div>
                  </div>
